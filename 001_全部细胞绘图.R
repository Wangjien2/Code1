...
#-------------------------------------------------------------------------------------------------------------
## 读入大群数据 ##
scRNA <- qread('/root/wangje/Project/OveryArtical/01AllCluster/05_大群数据_scp_new.qs')
scRNA
# An object of class Seurat 
# 33047 features across 178479 samples within 1 assay 
# Active assay: RNA (33047 features, 2000 variable features)
#  2 layers present: counts, data
#  7 dimensional reductions calculated: Harmonypca, Harmony, HarmonyUMAP2D, HarmonyUMAP3D, BBKNNpca, BBKNNUMAP2D, BBKNNUMAP3D
...
scRNA
# An object of class Seurat 
# 33047 features across 154529 samples within 1 assay 
# Active assay: RNA (33047 features, 2000 variable features)
#  2 layers present: counts, data
#  7 dimensional reductions calculated: Harmonypca, Harmony, HarmonyUMAP2D, HarmonyUMAP3D, BBKNNpca, BBKNNUMAP2D, BBKNNUMAP3D
## 设置celltype level ##
...
ggsave(p, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/样本细胞数.png", width = 6, height = 10,bg = 'white')
...
ggsave(p, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/样本细胞数分布图.png", width = 18, height = 5,bg = 'white')
...
p1 <- DimPlot2(scRNA,group.by = "celltype",reduction = "HarmonyUMAP2D",startNum = 0,xlab = "UMAP1",ylab = "UMAP2",split.by = NULL,ncol = NULL,label.size = 4,cols = RColorBrewer::brewer.pal(9,'Set1'))
p1 <- p1 + theme_classic()+
  theme_dr(xlength = 0.2, ylength = 0.2,arrow = arrow(length = unit(0.1, "inches"),type = "closed"))+
  theme(panel.grid = element_blank(),legend.title = element_blank())
# 保存图片
if(!dir.exists('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图'))dir.create('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图',recursive = TRUE)
ggsave(p1, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/DimPlot2_celltype.png", width = 6, height = 4,bg = 'white')
ggsave(p1, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/DimPlot2_celltype.pdf", width = 6, height = 4,bg = 'white')

## sample ##
p2 <- DimPlot(scRNA, group.by = 'sample',cols = c(RColorBrewer::brewer.pal(n = 8,name = 'Set1'),RColorBrewer::brewer.pal(n = 12,name = 'Paired')), reduction = "HarmonyUMAP2D")+
...
ggsave(p2, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/DimPlot_sample.png", width = 6, height = 4,bg = 'white')
ggsave(p2, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/DimPlot_sample.pdf", width = 6, height = 4,bg = 'white')
        
## Group ##   
print(unique(scRNA$Group1))   #[1] "39-44" "18-35" "37-39" "47-49"
p3 <- DimPlot(scRNA, group.by = 'Group1',cols = c('#dfb67d','#cb8eab','#869746','#4f8799'), reduction = "HarmonyUMAP2D")+
    ...
    theme(legend.title = element_blank(),panel.grid = element_blank())
ggsave(p3, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/DimPlot_Group1.png", width = 5, height = 4,bg = 'white')
ggsave(p3, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/DimPlot_Group1.pdf", width = 5, height = 4,bg = 'white')

#-------------------------------------------------------------------------------------------------------------
###### 绘制split dimplot #####
library(SCP)
# 绘制不同分组的celltype
p4 <- SCP::CellDimPlot(scRNA, group.by = 'celltype',reduction = 'HarmonyUMAP2D',split.by = 'Group1',palette = 'Set1',xlab = 'UMAP1',ylab = 'UMAP2',theme_use = 'theme_blank',bg_color = 'white',ncol = 4)
ggsave(p4, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/DimPlot_Group1_split.png", width = 24, height = 4,bg = 'white')
ggsave(p4, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/DimPlot_Group1_split.pdf", width = 24, height = 4,bg = 'white')

p4 <- SCP::CellDimPlot(scRNA, group.by = 'celltype',reduction = 'HarmonyUMAP2D',split.by = 'Group4',palette = 'Set1',xlab = 'UMAP1',ylab = 'UMAP2',theme_use = 'theme_blank',bg_color = 'white',ncol = 3)
ggsave(p4, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/DimPlot_Group4_split.png", width = 18, height = 4,bg = 'white')
ggsave(p4, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/DimPlot_Group4_split.pdf", width = 18, height = 4,bg = 'white')

# 绘制每个样本的celltype split
print(unique(scRNA$sample))
print(length(unique(scRNA$sample)))
p5 <- SCP::CellDimPlot(scRNA, group.by = 'celltype',reduction = 'HarmonyUMAP2D',split.by = 'sample',palette = 'Set1',xlab = 'UMAP1',ylab = 'UMAP2',theme_use = 'theme_blank',bg_color = 'white',ncol = 4)
ggsave(p5, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/DimPlot_sample_split.png", width = 30, height = 12,bg = 'white', limitsize = FALSE)
ggsave(p5, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/DimPlot_sample_split.pdf", width = 30, height = 12,bg = 'white',limitsize = FALSE)
...
# 保留获得的差异基因结果
write.table(scRNA.FindAllMarks,file = "/root/wangje/Project/OvaryFigs/01_全部细胞/热图/FindAllMarkers_onlyPos.txt",quote = FALSE,sep = '\t')
# 过滤差异基因
Top10.genes <- scRNA.FindAllMarks %>% group_by(cluster) %>% arrange(desc(avg_log2FC), p_val_adj) %>% dplyr::slice(1:10)
# 保存Top10基因的结果
write.table(Top10.genes,file = "/root/wangje/Project/OvaryFigs/01_全部细胞/热图/Top10_genes.txt",quote = FALSE,sep = '\t')
## 绘制热图 ##
# p6 <- DoHeatmap(scRNA,group.by = 'celltype',label = TRUE,use.clustering = TRUE,show.text = TRUE,angle = 90,cluster_cols = FALSE,cluster_rows = FALSE,save = TRUE,save.filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/热图/heatmap.pdf",save.width = 10,save.height = 10)

#-------------------------------------------------------------------------------------------------------------
# ---------------------------------------------------------------------
#  # Sat Jul  6 17:57:31 2024    绘制基因表达气泡图01
# ---------------------------------------------------------------------
library(viridisLite) # 选择使用 viridisLite
library(ggplot2)
library(Seurat)
library(rlang)
...
create_dot_plot <- function(seurat_object,
  ...
  if (!inherits(seurat_object, 'Seurat')) {
    stop("输入的对象不是一个Seurat对象")
  }
  
  DefaultAssay(seurat_object) <- assay %||% "RNA"
  plot_data <- Seurat::DotPlot(seurat_object, group.by = group_by, features = markers,...)$data
  ...
  base_theme <- theme_bw() +
    theme(
      ...
      legend.title = element_text(size = size, colour = "black"),
      axis.ticks = element_line(linewidth = 1),
      axis.ticks.length = unit(0.2, 'cm'),
      panel.border = element_rect(linewidth = 0.8, color = "black"),
      panel.grid.major = element_line(colour = "lightgrey", linewidth = unit(0.15, 'mm')),
  ...
  if (!facet) {
    plot <- plot_data %>%
      ...
      theme(
        ...
        axis.text.x = element_text(hjust = hjust, vjust = vjust, angle = angle, colour = "black", size = 18),
        axis.text.y = element_text(colour = "black", size = size),
        legend.key.size = unit(1, "lines"),
        legend.title.align = 0.5
      ) + 
      (if (use_viridis) scale_fill_viridis_c() else  
        scale_fill_gradientn(colors = RColorBrewer::brewer.pal(n = 9, name = palette))) 
    
  } else {
    plot <- Seurat::DotPlot(seurat_object, assay = "RNA", features = markers, scale = TRUE, group.by = group_by) +
      base_theme +
      theme(
        axis.text.x = element_text(angle = angle, vjust = vjust, hjust = hjust, size = 14, color = "black"),
        axis.text.y = element_text(size = 15, color = "black"),
        strip.text = (if (show_strip_text) element_text(size = 14) else element_blank()),
        strip.background = element_rect(fill = "white", color = NA)
      )+
      (if (use_viridis) scale_color_viridis_c() else  
        scale_color_gradientn(colors = RColorBrewer::brewer.pal(n = 9, name = palette))) 
  }
  plot <- plot + 
    labs(x = "", y = "", 
...
# mark_list <- list(
#     # Immune = c("PTPRC"),
#     NK = c("KLRC1", "FCGR3A", "NCAM1", "KLRD1"),
#     "T cells" = c("CD3G","CD8A", "CD8B", "CD4"),
...
#     "Smooth muscle" = c("GJA4", "PLAC9", "CRYAB", "ACTA2"),
...
p1 <- create_dot_plot(
  ...
  show_strip_text = FALSE,
  palette = "RdYlGn")
ggsave(p1, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞marker基因气泡图.png", width = 10, height = 4,bg = 'white', limitsize = FALSE)
ggsave(p1, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞marker基因气泡图.pdf", width = 10, height = 4,bg = 'white',limitsize = FALSE)
...
plist <- plotMarkerFeatures(scRNA, mark_list = mark_list, reduction = 'HarmonyUMAP2D')
ggsave(patchwork::wrap_plots(plist,ncol = 6), filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞marker基因FeaturePlot图.png", width = 24, height = 24,bg = 'white', limitsize = FALSE)
# ggsave(patchwork::wrap_plots(plist,ncol = 6), filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞marker基因FeaturePlot图.pdf", width = 10, height = 4,bg = 'white',limitsize = FALSE)
...
for(i in names(mark_list)){
  for(j in mark_list[[i]]){
    if(j %in% rownames(scRNA)){
      cat(j,'is runing...\n')
      plist[[paste0(i,'|',j)]] <- SCpubr::do_NebulosaPlot(
        scRNA, 
...
ggsave(patchwork::wrap_plots(plist,ncol = 6), filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞marker基因密度图.png", width = 28, height = 24,bg = 'white', limitsize = FALSE)
...
# 写出文件
write.csv(df,'/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞比例变化.csv',row.names = FALSE, quote=FALSE)
# 去除其中的卵细胞和颗粒细胞
...
# 保存图片
ggsave(ggpubr::ggarrange(plotlist = plist,ncol = 4,nrow = 2,common.legend = TRUE, legend = 'bottom'), filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞比例箱线图.png", width = 12, height = 8,bg = 'white', limitsize = FALSE)
ggsave(ggpubr::ggarrange(plotlist = plist,ncol = 4,nrow = 2,common.legend = TRUE, legend = 'bottom'), filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞比例箱线图.pdf", width = 12, height = 8,bg = 'white', limitsize = FALSE)
...
# 写出文件
write.csv(df,'/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞比例变化Group2.csv',row.names = FALSE, quote=FALSE)
# 去除其中的卵细胞和颗粒细胞
df <- df[!(df$celltype %in% c('Oocyte','Granulosa')),]
if(is.factor(df$celltype)) df$celltype <- varhandle::unfactor(df$celltype)
# 运行函数
...
# 保存图片
ggsave(ggpubr::ggarrange(plotlist = plist,ncol = 4,nrow = 2,common.legend = TRUE, legend = 'bottom'), filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞比例箱线图Group2.png", width = 10, height = 8,bg = 'white', limitsize = FALSE)
ggsave(ggpubr::ggarrange(plotlist = plist,ncol = 4,nrow = 2,common.legend = TRUE, legend = 'bottom'), filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞比例箱线图Group2.pdf", width = 10, height = 8,bg = 'white', limitsize = FALSE)
...
# 写出文件
write.table(or,file = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype_Group1_OR.txt",quote = F,sep = "\t",row.names = F)
write.table(or2,file = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype_Group4_OR.txt",quote = F,sep = "\t",row.names = F)
###### 绘制OR值热图 #####
...
# 绘图并保存文件
png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype_Group1_OR值热图.png', height = 6,width = 5, unit='in', res=300) 
Plot_OR(or)
dev.off()

png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype_Group1_OR值热图.pdf', height = 6,width = 5, unit='in', res=300) 
Plot_OR(or)
dev.off()

png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype_Group4_OR值热图.png', height = 6,width = 5, unit='in', res=300) 
Plot_OR(or2)
dev.off()

png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype_Group4_OR值热图.pdf', height = 6,width = 5, unit='in', res=300) 
Plot_OR(or2)
dev.off()
...
write.table(as.data.frame(Roe), file = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype_Group1_Roe值.txt" , sep = '\t', quote = F, row.names = F)
write.table(as.data.frame(Roe2), file = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype_Group4_Roe值.txt" , sep = '\t', quote = F, row.names = F)
...
png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype_Group1_Roe值热图.png', height = 6,width = 7, unit='in', res=300) 
p4
dev.off()

png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype_Group1_Roe值热图.pdf', height = 6,width = 7, unit='in', res=300) 
p4
dev.off()
...
png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype_Group4_Roe值热图.png', height = 6,width = 7, unit='in', res=300) 
p4
dev.off()

png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype_Group4_Roe值热图.pdf', height = 6,width = 7, unit='in', res=300) 
p4
dev.off()
...
p <- ggtexttable(df, rows = NULL)
ggsave(p, filename = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/celltype样本细胞数.png", width = 4, height = 4,bg = 'white')
...
head(prop)
# 保存文件
write.table(prop, file = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/堆叠柱形图sample_celltype.txt", sep = "\t", quote = F, row.names = F)
...
p1 <- plot_alluvial_barplot(data = prop, 
                      ...
                      split.by = "Group1",
                      use_ggh4x = TRUE)
ggsave(plot = p1, file = '/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/堆叠柱形图sample_celltype.png', height = 7,width = 15,bg = 'white', dpi = 300,limitsize = FALSE)
ggsave(plot = p1, file = '/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/堆叠柱形图sample_celltype.pdf', height = 7,width = 15,bg = 'white', dpi = 300,limitsize = FALSE)
...
head(prop)
# 保存文件
write.table(prop, file = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/堆叠柱形图sample_celltype_Group4.txt", sep = "\t", quote = F, row.names = F)
...
p1 <- plot_alluvial_barplot(data = prop, 
                      ...
                      split.by = "Group4",
                      use_ggh4x = TRUE)
ggsave(plot = p1, file = '/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/堆叠柱形图sample_celltype_Group4.png', height = 7,width = 12,bg = 'white', dpi = 300,limitsize = FALSE)
ggsave(plot = p1, file = '/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/堆叠柱形图sample_celltype_Group4.pdf', height = 7,width = 12,bg = 'white', dpi = 300,limitsize = FALSE)
#-------------------------------------------------------------------------------------------------------------
### 分组与celltype
### Group1 4个分组
...
head(prop)
# 保存文件
write.table(prop, file = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/堆叠柱形图Group1_celltype.txt", sep = "\t", quote = F, row.names = F)
p1 <- plot_alluvial_barplot(data = prop, 
                      x = 'Group1', 
                      y = 'Freq', 
                      fill_var = 'celltype',
                      ...
                      split.by = "Group1",
                      use_ggh4x = FALSE)
ggsave(plot = p1, file = '/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/堆叠柱形图Group1_celltype.png', height = 7,width = 6,bg = 'white', dpi = 300,limitsize = FALSE)
ggsave(plot = p1, file = '/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/堆叠柱形图Group1_celltype.pdf', height = 7,width = 6,bg = 'white', dpi = 300,limitsize = FALSE)               
...
head(prop)
# 保存文件
write.table(prop, file = "/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/堆叠柱形图Group4_celltype.txt", sep = "\t", quote = F, row.names = F)
p1 <- plot_alluvial_barplot(data = prop, 
                      x = 'Group4', 
                      y = 'Freq', 
                      fill_var = 'celltype',
                      ...
                      split.by = "Group1",
                      use_ggh4x = FALSE)
ggsave(plot = p1, file = '/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/堆叠柱形图Group4_celltype.png', height = 7,width = 5,bg = 'white', dpi = 300,limitsize = FALSE)
ggsave(plot = p1, file = '/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/堆叠柱形图Group4_celltype.pdf', height = 7,width = 5,bg = 'white', dpi = 300,limitsize = FALSE)  
...
# 获得表达值
features <- c('AKT1', 'AKT2','AKT3', 'CCND1', 'CCND2', 'CCND3', 'CCNE1', 'CCNE2', 'MYC')
...
png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞增殖和生长调控基因表达.png',height = 5,width = 22,units = 'in', res=300)
p1 + p2 + p3 + p4 
dev.off()
#-------------------------------------------------------------------------------------------------------------
#细胞周期基因
features <- c('CDC25A', 'CDK1','CDK2', 'CDK4', 'CDK6', 'CDKN1A', 'CDKN2A', 'CDKN2B')
scRNA$new_group <- paste0(scRNA$celltype,'--',scRNA$Group1)
df <- as.data.frame(AverageExpression(scRNA, features = features,slot = 'data',group.by = 'new_group',return.seurat = FALSE)$RNA)
df_18_35 <- t(df %>% dplyr::select(contains("18-35")))
rownames(df_18_35) <- stringr::str_split_fixed(rownames(df_18_35),pattern = "--",n = 2)[,1]
...
png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞周期基因.png',height = 5,width = 22,units = 'in', res=300)
p1 + p2 + p3 + p4 
dev.off()

#-------------------------------------------------------------------------------------------------------------
# 信号传导和代谢调节基因
features <- c('PIK3CA', 'PIK3CB', 'PIK3CD', 'PIK3R1', 'PIK3R2', 'PIK3R3', 'KRAS', 'NRAS', 'HRAS', 'FOXO1', 'FOXO3', 'MTOR','MAP2K1', 'MAP2K2', 'MAP2K3', 'MAP2K6', 'MAPK1', 'MAPK11',
...
png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/信号传导和代谢调节基因.png',height = 5,width = 30,units = 'in', res=300)
p1 + p2 + p3 + p4 
dev.off()
#-------------------------------------------------------------------------------------------------------------
# 抑制细胞衰老的基因
...
png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/抑制细胞衰老的基因.png',height = 5,width = 15,units = 'in', res=300)
p1 + p2 + p3 + p4 
dev.off()
...
png('/root/wangje/Project/OvaryFigs/01_全部细胞/UMAP图/细胞衰老诱导基因.png',height = 5,width = 15,units = 'in', res=300)
p1 + p2 + p3 + p4 
dev.off()
...
#                            富集分析
#-------------------------------------------------------------------------------------------------------------
## 这里使用去除其他混杂细胞的细胞群进行分析 ##
Fib <- qread('/root/wangje/Project/OveryArtical/04Fibroblasts/001_成纤维细胞.qs')
